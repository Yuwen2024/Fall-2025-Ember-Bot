...
static esp_err_t video_page_handler(httpd_req_t *req) {
    ...
    <head>
      ...
      <style>
        ...
        img {
          width: 100%;
          height: 100%;
          object-fit: contain;  /* scale the video to fit the container */
          display: block;
        }
      </style>
    </head>
...
void startCameraServer() {
    ...
    if (httpd_start(&stream_httpd, &config) == ESP_OK) {
        ...
        // HTML page that displays the stream (scaled)
        httpd_uri_t video_uri = {
            .uri       = "/video",
            .method    = HTTP_GET,
            .handler   = video_page_handler,
            .user_ctx  = NULL
        };
        httpd_register_uri_handler(stream_httpd, &video_uri);

        // Optional: root page redirect
        httpd_uri_t index_uri = {
            .uri       = "/",
            .method    = HTTP_GET,
            .handler   = [](httpd_req_t *req) -> esp_err_t {
                return httpd_resp_set_status(req, "303 See Other") == ESP_OK &&
                       httpd_resp_set_hdr(req, "Location", "/video") == ESP_OK &&
                       httpd_resp_send(req, NULL, 0) == ESP_OK
                       ? ESP_OK : ESP_FAIL;
            },
            .user_ctx = NULL
        };
